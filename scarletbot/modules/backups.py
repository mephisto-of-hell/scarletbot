...
# from scarletbot.modules.sql import warns_sql as warnssql
import scarletbot.modules.sql.blacklist_sql as blacklistsql

# from scarletbot.modules.sql import cust_filters_sql as filtersql
# import scarletbot.modules.sql.welcome_sql as welcsql
import scarletbot.modules.sql.locks_sql as locksql
import scarletbot.modules.sql.notes_sql as sql

# from scarletbot.modules.rules import get_rules
import scarletbot.modules.sql.rules_sql as rulessql
from scarletbot import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from scarletbot.__main__ import DATA_IMPORT
from scarletbot.modules.connection import connected
from scarletbot.modules.helper_funcs.alternate import typing_action
from scarletbot.modules.helper_funcs.chat_status import user_admin
from scarletbot.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("scarletbot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("scarletbot{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `scarletbot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("scarletbot{}.backup".format(chat_id))  # Cleaning file
